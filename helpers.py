...
def plot_accuracy_magnitude(history, acc_lim = [0.25, 1]):
    fig, ax1 = plt.subplots()

    test_accuracy = [x['test_accuracy'] for x in history]
    train_accuracy = [x['train_accuracy'] for x in history]
    magnitude = [x['magnitude'] for x in history]

    # plot test_accuracy, train_accuracy and magnitude. First two on the left axis, magnitude on the right axis
    ...
    ax2.set_ylabel('Magnitude')  # we already handled the x-label with ax1
...
def create_nn(layers):
    ...
    for i in range(len(layers) - 1):
        ...
        # Add a ReLU layer after each Linear layer except the last one
        if i < len(layers) - 2:
